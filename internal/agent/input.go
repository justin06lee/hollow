...
func (s *Server) input(in api.Input) error {
	...
	case api.InputDblClick:
		...
		args = append(args, "click", "--repeat", "2", "--delay", "80", btn)
	case api.InputDown:
		if err := moveTo(in.X, in.Y); err != nil {
			return err
		}
	...
	case api.InputType:
		...
		return s.xdotool([]string{"type", "--delay", "8", "--file", "-"}, in.Text)
	case api.InputKey:
	...
	case api.InputDrag:
		...
		args = append(args, "mousedown", btn, "sleep", "0.15",
			"mousemove", "--sync", strconv.Itoa(*in.ToX), strconv.Itoa(*in.ToY), "sleep", "0.15",
			"mouseup", btn)
	default:
